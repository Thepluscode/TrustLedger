...
| Exception-operations upgrade (ownership, exposure, deadline, activity, role permissions/APIs) | **PLANNED — GATED** | Existing issue classification, evidence, audit and atomic resolution remain the foundation. Implementation is deliberately not started while the market gate is 0/25; it unlocks only after `score_kill_test.py` returns `GO`, as required by the approved plan. |
...
| Outcome | Status | Evidence |
|---|---|---|
| First-three-conversation milestone | **IN PROGRESS — 0/3** | `pilot/FIRST_THREE_CONVERSATIONS.md` defines the non-demo incident-reconstruction interview, required quantitative evidence, data/paid follow-ups and immediate tracker update. `score_kill_test.py` reports completed conversations and qualified interviews separately; a booked call or public signal still counts as zero. |
