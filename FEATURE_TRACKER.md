...
|---------|--------|----------|
| Governed import: raw file stored before derivation, manifest, row-level rejection, idempotent re-import | **VERIFIED (local, PostgreSQL)** | `CaseImportIntegrationTest` 10/10. Raw bytes land in write-once `evidence_objects` (V50) before any row is parsed; same bytes again = replay, 0 new rows; a file refused as a whole commits a FAILED manifest with 0 rows and blocks the run until discarded. |
| Deterministic engine: 5 staged rules, 14 finding types, no fuzzy or AI matching | **VERIFIED (local)** | `EngineRulesTest` 19/19, `AcmeEngineAcceptanceTest` 5/5 with §20 numbers hard-coded. 7 mutants killed (incl. input-sort removal and lookup by unnormalised amount). |
| Engine scale | **MEASURED** | `EngineScaleTest`: 9,950 records 142 ms; 199,000 records ~3.1 s run alone (39 s when the host was swapping). The first measurement found a real O(n²) pairing loop that never finished at 100k payments; fixed with keyed lookup, same rule semantics. |
| Runs: atomic, replay on identical inputs + rules + fee schedules, findings raised once into the existing exception queue | **VERIFIED (local, PostgreSQL)** | `AcmeAcceptanceIntegrationTest` 5/5 over HTTP: 30 records, 10 of 11 matched (0.9091), R1=9 R2=1 R3=6 R4=0, 7 exceptions, GBP 50.60 and NGN 45000.00 never combined, provider-a reported as settlement NOT checked. Injected mid-run failure leaves 0 runs, 0 totals, 0 exceptions. 5 mutants killed. |
| Exception lifecycle: 6 states, closed transition table, reason codes, evidence required for WRITTEN_OFF / PROVIDER_CORRECTED, optimistic version check, append-only history | **VERIFIED (local, PostgreSQL)** | `ReconciliationIssueLifecycleIntegrationTest` 8/8; 8 concurrent closes → exactly 1 decision; DB refuses UPDATE/DELETE on history and refuses the two status columns disagreeing. 4 mutants killed (the first survived until an owned-exception case was added). `status` stays the coarse OPEN/RESOLVED flag for ~10 existing consumers; `lifecycle_state` is new (V54). |
| Deliberate behaviour changes to existing endpoints | **IMPLEMENTED** | Foreign-tenant read of an exception is now 404, not 403 (existence no longer leaks; 2 assertions updated). Assign/resolve need `RECON_ISSUE_WORK` / `RECON_ISSUE_RESOLVE` (admin roles hold them; new `RECON_OPERATOR` role). WRITTEN_OFF without attached evidence is refused; the old console offered it bare. |
| Evidence bundle `recon-case-bundle/1`: deterministic `contentHash`, INTERIM/FINAL, limitations block | **VERIFIED (local, PostgreSQL + independent verifier)** | `CaseBundleIntegrationTest` 4/4 against §20 counts. `scripts/verify_recon_bundle.py` (no shared code) reproduced the hash of a real export, failed an edited copy, failed a non-source file. 2 mutants killed. 24 existing evidence tests green. |
| Migration upgrade over existing data (V50–V55) | **VERIFIED (local)** | `verify-migrations-over-existing-data.sh upgrade_check 38`: 4 pre-V54 exceptions carried to OPEN / RESOLVED+RECOVERED / DISMISSED+FALSE_POSITIVE / RESOLVED with no reason invented; breaking the V54 mapping turns it red. |
| Operator console: cases, import, rejections, run results, exception workflow, bundle export | **VERIFIED (live local browser)** | Real backend + built console, seeded by `scripts/seed_acme_case.sh`: case page showed the §20 numbers; one exception taken OPEN → ASSIGNED → INVESTIGATING → RESOLVED with attached evidence; bundle exported. 0 console errors, no overflow at 390 px. Found and fixed: import table overflow hid the actions column. Typecheck, 9 Vitest tests, production build pass. |
| Canonical event ingestion: a webhook is a one-row import; files and live events reconcile on one path | **VERIFIED (CI, 2026-09-22)** | `279377f`. `AcmeFeedConvergenceIntegrationTest` 4/4: provider-b delivered as 7 JSON events + 3 files → the §20 numbers (30 records, 10/11, R1=9 R2=1 R3=6, 7 exceptions, GBP 50.60 / NGN 45000.00), every event-sourced exception cites the stored body by hash; 3 redeliveries → 0 new records, `delivery_count` 4; re-send with changed bytes → 1 `DUPLICATE_PROVIDER_EVENT`, no doubled effect; late event reopens the case and matches on rerun; bad/revoked token, unknown feed, closed case, empty/oversized body → envelope only, nothing under any tenant; feeds tenant-scoped. Ruleset 1.1.0: matched PENDING → `PENDING_UNKNOWN` (classification UNKNOWN), never a manufactured mismatch. 4 mutants killed (token check, transport dedup, raw-before-parse, PENDING-as-mismatch). CI 8/8 on the pushed branch. Not production-observed; no provider adapter wired (canonical JSON shape only, by design). |
| Frontend test runner in CI; react / react-dom version skew | **IMPLEMENTED; CI pending** | Vitest wired into the frontend job (slice 1). First run exposed react 19.2.8 vs react-dom 19.2.6, a real pre-existing defect; react-dom bumped. |
| Flyway SSL context-start flake | **STILL UNEXPLAINED** | `scripts/flyway-ssl-flake-probe.sh`: 60 connection attempts (30 `prefer`, 30 `disable`), 0 failures on an idle 2 GB colima. The port-forwarder race hypothesis is not supported. It has never appeared in CI; host memory pressure is the leading suspect, unproven. No retry was added. |
| Full backend suite on this branch | **NOT RUN LOCALLY** | The full local suite drove this 8 GB host to 10.7 GB swap and 441 MB free disk and was abandoned; only scoped runs were made (largest: 91 tests green). CI on the pushed branch is the only complete verdict. |
| Pre-existing defect found, not fixed (out of scope) | **OPEN** | `POST /api/v1/transfers` without an `Idempotency-Key` header returns 500 instead of 400 (`MissingRequestHeaderException` is unmapped). `EvidenceService.requireExportInScope` still answers 403 for a foreign export id rather than 404. |

## Lead walkthrough bottlenecks (2026-08-20)

| Outcome | Status | Evidence |
|---------|--------|----------|
| Reconciliation owner assignment in the operator UI | **VERIFIED (local)** | A fresh tenant ingested synthetic statement `LEAD-WALKTHROUGH-2026-08-20`; the real settlement detector raised one OPEN `SETTLEMENT_LINE_UNMATCHED` issue with £125 exposure. In the live console, the owner control assigned the tenant owner, unassigned it, then reassigned it. The owner field showed the member email and the activity register retained all three audited transitions. Browser console: 0 errors/warnings. `ReconciliationExceptionOpsIntegrationTest`: 8/8 against PostgreSQL, including tenant isolation, resolved-case rejection and unassignment history. No customer data or external provider action was used. |
| Reproducible read-only lead seed | **VERIFIED (live local API)** | `BASE=http://localhost:8090 ./pilot/demo-wedge-seed.sh` ran against a fresh PostgreSQL 16 database migrated through V49. It created a new tenant, ingested synthetic statement `LEAD-WALKTHROUGH-0f5688aa`, asserted exactly one unmatched row, found OPEN issue `45f1138f-a9e0-49b8-a4c6-744d6a33073a`, verified GBP 125 exposure, assigned the tenant owner, verified `RECONCILIATION_ISSUE_ASSIGNED` in activity and re-authenticated the printed login. Exit 0; no payment API was called. `pilot/HOSTED_DEMO.md` and `pilot/DEMO_SCRIPT.md` now use this wedge seed, with the execution/fraud seed explicitly optional and separate. |
| Statement-to-exception traceability | **VERIFIED (live local)** | Settlement detail now returns tenant-scoped related exception IDs and renders direct `Open exception …` links. PostgreSQL integration coverage passed 9/9, including a new assertion that the seeded statement returns its one `SETTLEMENT_LINE_UNMATCHED` issue and another tenant receives 403; the complete backend regression then passed 522/522 with zero failures, errors or skips. `npx tsc --noEmit` and the 26-route production build passed. In the running console at 1280px, the seeded statement showed `1 reconciliation exception raised`; one click opened the correct HIGH/OPEN issue with £125 exposure, owner, due time, source-statement link and assignment activity. Document width equalled viewport width (1280px) and browser logs contained no warnings or errors. |
| Lead/onboarding failure transparency | **VERIFIED (live local failure injection)** | Against a fresh read-only wedge tenant, stopping the backend made Onboarding name `Unable to assess onboarding readiness: Failed to fetch`, render `Assessment unavailable`, state that no completion was inferred and withhold the checklist. This live check found and fixed a contradictory `0 of 0 / Core reliability setup is complete` state plus an indefinitely loading skeleton. A second isolated request-abort check proved the new `Retry assessment` recovery: the failed state contained no completion claim; after restoring the API and clicking Retry, it returned to `4 of 4` without reloading. Aborting only the issue-audit request kept the real £125 exception visible while rendering `Activity unavailable: Failed to fetch`; restoring the request brought its Activity register back. A clean-browser `/login` check also exposed and fixed a missing `/favicon.ico` 404 by adding the application icon; a second fresh visit had zero console errors or warnings. `npx tsc --noEmit`, `npm run test:public-exposure` and `npm run build` passed; 26 application routes plus the icon asset generated. |
| Dependency-free domain validation | **VERIFIED** | `scripts/run_domain_validation.sh` now keeps the Spring/JPA-backed certification package out of the pure-domain `javac` source set. The advertised root command ran to `Domain acceptance validation passed`; repository validation also passed with 47 unique migrations through V49. `CLAUDE.md` now invokes the harness from the directory where it actually exists. |
| Integration-suite background-worker and database lifecycle isolation | **VERIFIED** | Scheduling remains enabled by default in the application but is disabled centrally for tests, and each test application context now closes before its private Testcontainers database stops. `ReconciliationExceptionOpsIntegrationTest` passed 8/8 with a clean shutdown in 21 seconds; the previous run took 45 seconds and emitted post-test database failures. The full real-container suite then passed 521/521 with 0 failures, 0 errors and 0 skipped in 11:01 across 79 database-backed contexts. |

| Commit-time regression, 2026-08-23 | **VERIFIED (local)** | Commits `0af6394`, `814b89c`, `c20aeec`, `09a81fa`, `4b053af`. Full real-container suite: `Tests run: 522, Failures: 0, Errors: 0, Skipped: 0`, `BUILD SUCCESS`, 118 classes started / 118 surefire reports written, 05:48. Frontend: `npx tsc --noEmit` exit 0, `npm run build` exit 0 (27 routes), `npm run test:public-exposure` passed. `pilot/check_send.py --selftest` 10/10; `scripts/run_domain_validation.sh` → `Domain acceptance validation passed`; `scripts/validate_repo.py` → 47 unique migrations, highest V49. Run in CI: `.github/workflows/ci.yml` executes `mvn test` on every push and pull request to main (five recent runs green as of 2026-09-07). Corrected 2026-09-07 — this line previously read 'Still not run in CI', which had been false for some time. **CI now also asserts the suite actually ran**: 78 of 79 @Testcontainers classes carry `disabledWithoutDocker=true`, so a runner without a Docker daemon skipped every integration test while `mvn test` exited 0 and the job went green. `backend/scripts/assert_test_floor.py` fails the build below 500 executed tests, 110 classes, or on any skip — observed failing against the real 522-test reports rewritten as fully skipped, and its own selftest is 9 checks. |
| Intermittent context-start failure, recorded not buried | **KNOWN FLAKE — unexplained** | The first full run of the same tree failed: `TenantRbacAndPolicyIntegrationTest` 2 errors, both `Failed to load ApplicationContext`, root cause `Flyway … Unable to obtain connection from database: An error occurred while setting up the SSL connection` — while a sibling context had migrated to v49 seconds earlier. Re-run in isolation: 2/2 green. Re-run of the whole suite: 522/522 green. So it is intermittent and its cause is **not** established; the plausible suspect is container churn now that each context closes after its class. It is recorded here because a failure that disappears on re-run is exactly the kind that teaches a team to re-run rather than look, and the next occurrence should be investigated, not retried. |

## Canonical product and pilot gate (2026-08-12)

| Outcome | Status | Evidence |
|---------|--------|----------|
| Canonical payment-reliability doctrine | **IMPLEMENTED** | `docs/CANONICAL_PRODUCT_DOCTRINE.md` now fixes the primary question (“What actually happened to the money?”), canonical problem statement, three-role operating model, qualification criteria, read-only boundary, truth policy and staged scope. `README.md`, `docs/PRODUCT_BLUEPRINT.md`, `docs/GOLDEN_WORKFLOW.md`, the one-pager, pilot package, pricing, demo and interface language point to the same wedge. Runtime check at 1280px observed `Review exceptions` as the global action and evidence→exception onboarding with no console warnings/errors or horizontal overflow; 390×844 observed `Overview / Exceptions / Evidence / More` and no horizontal overflow. |
| Market gate | **IN PROGRESS — BLOCKING** | `python3 pilot/score_kill_test.py` observed 0 conversations / 0 qualified interviews: pain 0/6, recurrence 0/4, data 0/3, paid 0/2, `INCOMPLETE`. Bars aggregate by company, `maybe` never passes data/money, and out-of-segment conversations remain recorded but excluded; 17 scorer self-checks pass. Per Rule 0 and the approved plan, this blocks new post-gate exception-operations infrastructure. |
| Six-week real-data product-gate protocol | **IMPLEMENTED; not run** | `pilot/PILOT_CHECKLIST.md` specifies the four-week baseline, 30+ labelled-case counterbalanced benchmark, two-week parallel run, four-week TrustLedger-primary run, role/data boundaries and exit evidence. Three header-only pseudonymous CSV templates hold benchmark, operating-day and paid-pilot evidence; no customer data is committed. |
| Reproducible product-gate scorer | **IMPLEMENTED** | `pilot/score_product_gate.py` fails closed unless the market gate is `GO`, then checks speed, accuracy, recall, evidence completeness, false closure, silent absorption, paid commitment, sustained usage, weekly management review, evidence-linked engineering escalation and four consecutive spreadsheet-displacement weeks. Its 24 self-checks exercise every independent bar plus malformed lifecycle/count evidence; Python compilation and repository validation pass. |
| Exception-operations upgrade (ownership, exposure, deadline, activity, role permissions/APIs) | **VERIFIED (local); customer proof gated** | The founder explicitly authorised this implementation while the market gate remained incomplete. V49, tenant-scoped APIs, atomic assignment/resolution, exposure-by-currency, per-issue deadlines and the operator UI are locally verified below. This does not pass the market or product gate and is not customer or production evidence. |
| Requirement-level completion audit | **IMPLEMENTED** | `docs/CANONICAL_PLAN_AUDIT.md` maps every plan section to authoritative evidence and records which outcomes are implemented, specified/gated or externally incomplete. It explicitly refuses to treat gate tooling as a passed market or product gate. |

## SELL / LEARN market queue (2026-08-12)

| Outcome | Status | Evidence |
|---|---|---|
| First-three-conversation milestone | **1 HELD, 0 RECORDED — 0/3** | The Exness call on 2026-08-31 took place (founder-confirmed 2026-09-06). An accuracy-check message went to Artem on 2026-09-02; the incident is not yet fully reconstructed and no data access was granted. `kill-test-tracker.csv` deliberately carries NO `date_interviewed` for that row: the scorer fails closed and requires all twelve interview fields plus four yes/no qualification answers first, and none of that content exists yet. The blank means evidence-not-recorded, not call-did-not-happen — stated in the row's notes and in `pilot/FIRST_THREE_CONVERSATIONS.md`. `score_kill_test.py` therefore still reports 0 of 25 (exit 3), and its selftest still passes. | Send the corrected anonymised reconstruction — the founder's Friday 2026-09-04 deadline has passed, so the keep-everything-unknown fallback is the live path. Only then may the interview date be set. |
| Market gate strengthened with recurrence | **IMPLEMENTED; not passed** | `score_kill_test.py` now requires 4 pain-confirming companies with monthly recurrence or materially quantified exposure, in addition to 6 pain, 3 data and 2 paid companies. The threshold was added while completed interviews remained 0, before results could influence it. `--selftest`: 17 checks pass, including recurrence failure, `maybe` non-passes, required incident evidence, company dedupe and out-of-segment exclusion. Live result remains 0 conversations / 0 qualified interviews. |
| Expanded interview evidence ledger | **IMPLEMENTED; empty** | `kill-test-tracker.csv` adds unique interview id, last incident, frequency, manual effort, exposure, systems checked, workaround, recurrence evidence, verbatim quote, next step and operational qualification. Missing evidence makes a completed row a data error; `unknown` preserves non-disclosure rather than inventing zero. Existing outreach rows remain intact. |
| PalmPay Interview #1 | **IN PROGRESS — outreach sent; awaiting reply** | Sales Navigator visibly confirmed the incident-first InMail to Friday Ogungbemi at 18:30 BST/WAT on 2026-08-14, displayed `Awaiting reply from LinkedIn Member` and reduced the InMail balance from 45 to 44. The message requested one anonymised recent discrepancy, offered three 20-minute slots, contained no product link or attachment and explicitly excluded a sales/demo call. `kill-test-tracker.csv` records the send and one permitted follow-up date. A sent message is not an interview; first-three and market-gate counts remain zero. |
| Active work queue changed from BUILD to SELL/LEARN | **IMPLEMENTED** | `pilot/SELL_LEARN_QUEUE.md` authorises target research → qualified conversation → incident reconstruction → tracker → data request → paid discovery, and explicitly rejects more product breadth until customer evidence or a critical defect requires it. Architecture resumes from the dominant recorded incident, not the old roadmap by default. |
| Thirty-day premise-exposure campaign | **IN PROGRESS — 2/39 published** | The Day 1 premise posts are live on LinkedIn (`urn:li:activity:7494067577850232832`, published 2026-08-14 16:29:03 UTC) and X (`2088302642267050152`, published 2026-08-14 16:32:03 UTC). The public LinkedIn page rendered the complete incident-first premise and four research hashtags; the founder supplied the canonical X status URL, whose text was not retrievable unauthenticated. `SOCIAL-CALENDAR.md` contains the remaining 37 assets and `pilot/social-exposure-tracker.csv` records publication URLs separately from operator outcomes. Publication creates distribution evidence only: qualified replies, completed conversations, datasets and paid commitments remain zero until independently recorded. |

## Interface-reference fidelity gate (2026-08-12)

| Feature | Status | Evidence |
|---------|--------|----------|
| Shared responsive operations shell + redesigned Dashboard, Transfer Detail and Fraud Case Queue | **VERIFIED (local)** | `npm run build` compiled all 25 frontend routes and completed TypeScript/static generation successfully. Observed in the live browser against a real local backend and a freshly seeded tenant: Dashboard showed 3 accounts, 3 completed transfers, 1 held transfer and 1 open case; Fraud Case Queue showed the held score-75 case and safe approve/reject controls; Transfer Detail showed the same £900 held transfer, lifecycle, review explanation and audit trail. Desktop checked at 1440×900; mobile checked at 390×844 with card layouts, bottom navigation and sticky review/create actions. Browser console: 0 errors/warnings. This verifies the three-screen premise gate only; the remaining route redesign rollout is not yet claimed. |
| Money + Reconciliation responsive redesign batch | **VERIFIED (local)** | Accounts, Transfers, Create Transfer, Ledger Explorer, Reconciliation Issue List/Detail and Settlement Statement List/Detail now follow the reference hierarchy on desktop and mobile while retaining the implemented workflows. A production build completed TypeScript and generated all 25 routes. Live checks against the local backend observed: 3 GBP accounts with £4,100 available kept separate from £900 reserved; the £900 score-75 `HELD FOR REVIEW` transfer with its safety explanation; a ledger transaction with £120 debit = £120 credit and `Balanced`; the three-step transfer risk/review flow with idempotency notice; and a synthetic SANDBOX statement that produced 0 matched/1 unmatched, one open reconciliation issue, expected-vs-actual evidence and the permissioned resolution form. Desktop checked at 1440×900; mobile checked at 390×844. Browser console: 0 errors (the only warning was Playwright's rejected first attempt to fill a `datetime-local` field with a date-only value; the corrected input succeeded). Backend `/api/health` returned `status: ok`. |
| Fraud remainder + Payment Rails responsive redesign batch | **VERIFIED (local, visual scope)** | Risk Profiles, ML Monitoring, Webhook Events, Provider Certification, Certification Run and Production Readiness now use restrained operational registers, split inspectors, consequence-led safety boundaries and compact mobile records instead of generic KPI/card dashboards. `npm run build` compiled, type-checked and generated all 25 routes. Live desktop/mobile checks observed one trusted device, beneficiary and user profile from real tenant data; ML correctly rendered an empty governed registry with an explicit shadow-mode/non-decision boundary rather than invented performance claims; Webhooks correctly rendered an empty accepted-event register with the replay-safe/no-double-post explanation; Production Readiness showed `Platform production execution DISABLED`, no production provider config, and no canary history. Provider Certification returned `Unauthorized` for this local owner session, so its populated/run-detail states remain implemented and build-verified but are not claimed as live-behaviour verified. No external provider or production action was performed. |
| Compliance + Developer + Organisation + access/onboarding responsive redesign batch | **VERIFIED (local, visual scope)** | Evidence Exports, Audit Logs, API Keys, Monitoring, Tenant Admin, Users & Roles, Org Units, Login and Onboarding now use compact operational registers, split task workspaces, explicit one-time-secret boundaries and data-derived readiness states. The login message is aligned to the read-only payment-reliability wedge; onboarding now says sandbox setup and explicitly avoids a production-readiness claim. `npm run build` compiled, type-checked and generated all 25 routes. A live owner session loaded all nine routes from the local backend; desktop checks at 1440×900 found no document overflow on the seven authenticated operational screens, and mobile checks at 390×844 found no overflow on Onboarding, Audit Logs, Monitoring and Org Units. Browser console contained 0 errors/warnings. Backend `/api/health` returned `status: ok`. Destructive role/key/plan actions and populated one-time-secret states were not executed during this visual verification. |
| Shared anti-template visual calibration | **VERIFIED (local, visual scope)** | The global console language is now industrial/utilitarian rather than decorative SaaS: flat deep-slate surfaces, one restrained cyan action colour, 4px panel geometry, square controls, register-style summary bands, monospaced state labels, and no gradient or shadow treatment on operational cards/panels. Product chrome now says `Payment reliability`; page metadata says `Payment Reliability Operations`; dashboard wording describes the current evidence position instead of a generic financial-control claim. `npm run build` compiled, type-checked and generated all 25 routes; live computed-style inspection at 1440×900 found 0 gradients across cards/panels/sidebar/topbar, 0 card/panel shadows, 4px panel radius and 0 horizontal overflow. Mobile Dashboard at 390×844 loaded all six real metrics with 0 horizontal overflow. Browser console: 0 errors/warnings. Backend `/api/health` returned `status: ok`. |

## Executive showcase layer (2026-08-12)

| Outcome | Status | Evidence |
|---|---|---|
| Six-scenario synthetic incident simulator | **VERIFIED (local, presentation scope)** | Authenticated `/showcase` presents missing settlement, duplicate event, provider/webhook conflict, fee overcharge, audit tamper and provider-timeout replays. The default £50,000 story retains all source states, calculates £325.25 expected versus £425.25 received, classifies `SETTLEMENT_FEE_MISMATCH` and labels £100 probable exposure. The UI permanently says synthetic, no customer data and no money movement; it separates repository evidence from customer proof. `npm run build` compiled/type-checked and generated all 26 routes. Browser observation at 1280×720 and 390×844 found no horizontal overflow and 0 console errors/warnings; the replay advanced to `EVIDENCE 2/5`, and the audit scenario concluded `VERIFICATION_FAILED`. This verifies the presentation layer, not live processing of each scenario. |
| Executive demo package | **IMPLEMENTED** | `docs/EXECUTIVE_SHOWCASE.md` contains a 90-second storyboard, five-minute talk track, one-page proof sheet, technical credibility sheet, honesty box and presenter guardrails. `pilot/DEMO_SCRIPT.md` now uses the same five-minute £50,000 narrative, and `pilot/HOSTED_DEMO.md` adds showcase smoke checks. No outreach was sent, video recorded or external application submitted. |
| Anonymous showcase boundary | **VERIFIED (local)** | `/showcase` no longer mounts the authenticated operations shell. It renders only hard-coded synthetic scenarios, makes no API/session/storage call, carries `noindex,nofollow`, and links separately to `/login` for the secure console. `npm run test:public-exposure` passed: the guard rejects shell/API/session/storage dependencies, requires the four honesty statements, and its injected-API negative control fired. `npm run build` generated all 26 routes. Browser observation at 1280×720 and 390×844 found no sidebar, operator email, redirect, horizontal overflow or console warnings/errors; mobile replay advanced to `EVIDENCE 2/5`. This exposes the route anonymously within the application; no public hosting deployment or DNS change is claimed. |
| EXPOSED capability register and operator workflow | **VERIFIED (local presentation scope)** | `docs/CAPABILITY_EXPOSURE_REGISTER.md` separates canonical main, open PR #125, local verification and external proof, while the public showcase exposes the same nine-domain register behind a collapsed disclosure. `npm run test:public-exposure` passed its dependency boundary and negative control; `npm run build` compiled, type-checked and generated all 26 routes. Live browser observation at 1280×720 found the register closed on first load and open with exactly 9 records after one click, with 0 horizontal overflow, no sidebar or operator email, and no warning/error log entries. `pilot/OPERATOR_EXPOSURE_WORKFLOW.md` keeps discovery incident-first and requires data/paid/kill evidence rather than feature praise. Repository validation passed. The market scorer still fails closed at 0 conversations: pain 0/6, recurrence 0/4, data 0/3 and paid 0/2; no operator was contacted. |
...
| Reference-image UI implementation | **IMPLEMENTED; local visual verification complete** | All 26 non-redirect reference route screens now have responsive implementations using the shared deep-slate/cyan operations language, real API-backed content and existing safety vocabulary. `npm run build` is green across all 25 generated Next.js pages. Representative and batch-specific live checks cover every route family at desktop and mobile breakpoints; Provider Certification populated/detail states still require a session with their required authority before live-behaviour verification can be claimed. |
...
| Backup / restore / DR drill | **VERIFIED (development restore only)** | A real PostgreSQL backup→drop→restore round-trip retained the tested dataset and 10/10 integrity invariants. `docs/RESTORE_TEST_RECORD.md` remains authoritative: off-host backup, PITR/WAL, object-store restore and lock-execution→provider-reconciliation→approved-resume have not been rehearsed, so controlled-pilot or production recovery is not claimed. The global `data-resilience-gate scan .` currently over-scores this repository as `PRODUCTION RECOVERY VERIFIED`; its keyword result contradicts the dated drill evidence and is recorded as a scanner false positive, not adopted as readiness proof. |
...
| Commercial pricing | **DONE; unvalidated** | `pilot/PRICING.md` now matches the approved company rate card exactly: £1,500 diagnostic, £4,000 discovery, first three £10,000 founding pilots, £20,000 standard pilot, exact production quote after discovery. No subscription is quoted before three pilots establish delivery cost. No signed deal validates these prices yet. |
