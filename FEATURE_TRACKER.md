...
- ~~**Environment-sensitive test (closed 2026-08-10):** `MonitoringService.nonNegativeAgeSeconds` floors computed age at 0 (`Math.max(0, …)`) — a negative result is always clock skew, never truth. Applied `@Testcontainers(disabledWithoutDocker = true)` to the 29 remaining integration tests not covered by PR #21 — see PR #128.~~
