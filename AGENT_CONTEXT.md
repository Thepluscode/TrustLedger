# AGENT_CONTEXT — TrustLedger

The stable charter: what this is, what it must never do, and the invariants. It overrides the
home/global doctrine inside `projects/fintech/TrustLedger_v2/`.

`CLAUDE.md` is the six-step bootstrap and points here. `ACTIVE_WORK.yaml` says what is
authorised right now. **No counts, no SHAs, no gate verdicts, no test totals live in this
file** — they belong in `PROJECT_STATE.json` (`make project-state`) and in
`FEATURE_TRACKER.md`, which carry their evidence. A number written here is stale the moment
the repository moves, and this file already proved it: it stated a test total that the suite
...
```bash
cd backend
mvn -B test            # JUnit 5 + Testcontainers. Needs Docker. CI is the only complete verdict.
mvn -B compile
...
**The verdict is generated, not written here** — run `~/.claude/scripts/data-resilience-gate scan .`; Rule 0.8 treats a drill older than 90 days as stale, so a figure typed into this file would age into a false claim. What the drill established, and what it did not, is recorded in `docs/RESTORE_TEST_RECORD.md`: primary database destroyed outright, restored in 0.8 s, **10/10 financial invariants held**, zero data loss against the backup point, and the validator itself proven to fail on a single deleted credit entry. `scripts/verify-restore-integrity.sh` is new and wired into the drill, because row counts are not integrity.
