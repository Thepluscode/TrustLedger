...
export default function OnboardingPage() {
  ...
  const loadReadiness = useCallback(async () => {
    ...
    setError(null);
    try {
      const [statements, issues, auditLogs, evidence, providers] = await Promise.all([
        api.listSettlementStatements(),
        api.listReconciliationIssues(),
        api.listAuditLogs(),
        api.listEvidence(),
      ...
      setSteps(null);
      setError(`Unable to assess onboarding readiness: ${(e as Error).message}`);
    }
  }, []);
  ...
  const progressLabel = steps === null ? (error ? "Assessment unavailable" : "Checking…") : `${coreDone} of ${coreTotal}`;
  const progressMessage = error
    ? "Readiness could not be assessed. No completion state has been inferred."
    : steps === null
      ? "Checking current tenant records."
      : coreDone === coreTotal
        ? "Core reliability setup is complete."
        : "Complete the evidence-to-exception path before reviewing optional controls.";
      ...
      <section className="panel onboarding-checklist">
        ...
        {steps === null ? (
          <div className="panelBody">
            {error
              ? <div>
                  <p className="sub">Checklist unavailable until current tenant data can be loaded.</p>
                  <button className="secondary" onClick={() => void loadReadiness()}>Retry assessment</button>
                </div>
