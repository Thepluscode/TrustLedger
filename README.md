...
case, and every sensitive action is auditable. **v2.2** added an external payment-rail sandbox; **v2.3** adds fraud intelligence (behaviour/device/beneficiary risk, case linking, dual approval) on top of
...
Backend: **76 tests, 0 failures** — pure-domain unit tests plus **Testcontainers** integration
across **PostgreSQL & Redpanda**.

```bash
cd backend && mvn test     # Tests run: 76, Failures: 0, Errors: 0, Skipped: 0  (2026-06-09)
...
```text
backend/    Spring Boot 4 + JPA + Flyway (db/migration/V1..V9) ; src/test = JUnit + Testcontainers
